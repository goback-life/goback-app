...
class FeedLockoutButton extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    // Match SVG viewBox proportions (86×102)
    final btnWidth = 86 * s;
      ...
      child: SizedBox(
        ...
        child: Stack(
          children: [
            ...
            // Layer 3: Painted overlay (tint, inner shadow, highlights)
            CustomPaint(
              size: Size(btnWidth, btnHeight),
              painter: _GlassOverlayPainter(glowIntensity: glowValue),
            ),
          ],
        ),
      ),
...
///
/// Highlights sit on the upper edge (NW light) and the bottom-right
/// corner (refracted light). The lower edge near the tip stays dark.
class _GlassOverlayPainter extends CustomPainter {
  _GlassOverlayPainter({this.glowIntensity = 0.0});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // 4. Painted edge refraction glow — thick blurred bright stroke
    //    near the edges, simulating increased distortion at the meniscus.
    //    Uses the same highlight zones so it reinforces them.
    canvas.drawPath(
      path,
      Paint()
        ..style = PaintingStyle.stroke
    ...
    // 5a. Direct reflection — NW light (315°) hitting the glass surface.
